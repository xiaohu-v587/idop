		...
		<div title="center" region="center" style="border:0px;">
				...
				<table style="width:100%;">
					<tr>
						<td style="width:100%;">
							...
							<a class="mini-button" iconCls="" id="startBtn" onclick="startOrStop()">启用/停用</a>
							<a class="mini-button" iconCls="" id="removeBtn" onclick="del()">删除</a>
							<a class="mini-button" iconCls="" id="cancel_removeBtn" onclick="cancelRemove()" visible="false">取消</a>
							<a class="mini-button" iconCls="" id="confirm_removeBtn" onclick="confirmRemove()" visible="false">确定</a>
							<a class="mini-button" iconCls="" id="addBtn" onclick="init()" >刷新缓存</a>
						</td>
						<td style="white-space:nowrap;">
							<input id="key1" class="mini-combobox" style="width:150px;" textfield="name" valuefield="key" emptyText="字典名称"
			...
			<div class="mini-fit">
				<div id="datagrid1" dataField="data" class="mini-datagrid" style="width:100%;height:100%;" sortMode="client" allowUnselect="false"
					url="<%=root%>/param/getList" oncellclick=""  onselectionchanged="onSelectionChanged"  onload="controlBtnState()"
...
<script type="text/javascript">
	...
	function startOrStop(){
		...
		if(status == "1"){
			msg="确定停用？";
			url = "<%=root%>/param/changeStatus1";
		}else if (status == "0"){
			msg="确定启用？";
			url = "<%=root%>/param/changeStatus0";
		}
	...
	function init(){	
		$G.GcdsConfirm("确定刷新缓存吗？", "刷新提示", function(action) {
			if (action == 'ok') {
                ...
                $G.postByAjax({}, "<%=root%>/param/init", ajaxConf);
              }
		});
	...
	function del(){
		...
		$G.GcdsConfirm("确定删除选中记录？", "删除提示", function(action) {
			if(action !="cancel" && action != "close"){
	            ...
	            $G.postByAjax({"id":id}, "<%=root%>/param/delete", ajaxConf);
			}else{
				cancelRemove();
			}
