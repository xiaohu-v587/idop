 ...
 * 
 */
@RouteBind(path = "/pccm_finance")
@Before({ ManagerPowerInterceptor.class })
public class PccmFinanceCtl extends BaseCtl {
	/**
	 * 进入首页
	 */
	@Override
	...
	public void getList() {
		...
		String sql = "select *";
		String extrasql = " from pccm_finance where 1=1";
		...
		if (ids.contains(",")) {
			...
			for (int i = 0; i < array.length; i++) {
				...
				Db.use("default").update(
						"delete from pccm_finance where id=?",
						new Object[] { uuid });
			}
		} else {
			Db.use("default").update("delete from pccm_finance where id=?",
					new Object[] { ids });
		}
		LoggerUtil.getIntanceof().saveLogger(
				getCurrentUser().getStr("USER_NO"), "财务报表挖掘", "6",
				"财务报表挖掘-删除");
		renderNull();
	}
}
