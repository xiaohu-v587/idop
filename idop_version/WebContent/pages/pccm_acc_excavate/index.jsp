...
<html>
	...
	<body>
		<div id="panel1" class="nui-panel" title="对账单挖掘" style="width:100%;" showToolbar="false" showCollapseButton="false"
		   		...
		   		<form id="form1">
			   		<table style="table-layout: fixed;" class="search_table" width="100%">
	               		<tr>
							...
							<td align="left">
								<input name="label_flag" class="nui-textbox" required="true"/>   
							</td>
	               		</tr>
			   		</table>
		   		</form>
			...
			<div class="nui-fit">
		    	<div id="datagrid1" dataField="data" class="nui-datagrid" style="width:100%;height:100%;" url="<%=root%>/pccm_acc_excavate/getList" multiSelect="true">
		...
		if(rows.length>0){
			$G.GcdsConfirm("确定删除选中记录？", "删除提示", function(action) {
				if (action == 'ok') {
                	...
                	$G.postByAjax({"ids":ids}, "<%=root%>/pccm_acc_excavate/del", ajaxConf);
              	}
            });
		}else{
			$G.alert("请选中一条记录");
