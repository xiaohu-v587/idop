...
<div class="main_box" style="overflow:auto;">
   <div class="content_box" style="width:66%;height:45%; background-color:#ffffff">
	...
	<div class="content_box" style="width:32.5%;height:45%;margin-left:0.5%; background-color:#ffffff">
                   ...
                   <ul class="tr3" >
               		...
               		<ul class="tr3" >
						...
						<li align="left">
							...
							<div id="warningname" name="warningname"  class="mini-autocomplete" valueField="warning_name" textField="warning_name" url="<%=root%>/panorama/markSearch" 
				        		style="border:0;"  >
 	...
 	<!--		<table class="table_1" style="margin-top:8%;margin-left:4%;font-size:10pt;">
					<tr class="tr3">	
               			<th align="right">触警时间：</th>
						<td align="left" >
							<input class="nui-datepicker" name="startime" id="startime" />
						</td>
						<th align="center">至</th>
						<td align="left" >
							<input class="nui-datepicker" name="endtime" id="endtime" />
						</td>
                   </tr>
                   <tr class="tr3">	
               			<th align="right">机构：</th>
						<td align="left">
							<input id="orgid" class="mini-treeselect" url="<%=root%>/org/getListByUser?noliketype=1" dataField="datas" 
								name="orgid" textfield="orgname" valuefield="id" parentfield="upid"  
								valueFromSelect="false" multiSelect="false"  expandOnLoad="0"
								allowInput="false" showClose="true" 
								showRadioButton="true" showFolderCheckBox="false" popupWidth="305"
                   ...
                   <tr class="tr3">
						<th align="right">预警等级：</th>
						<td align="left">
							<input name="warninglvl" id="warninglvl" class="nui-dictcombobox" textfield="dictName" valuefield="dictID" dictTypeId="dop_warning_lvl" value="" />
						</td>
               		</tr>
               		<tr class="tr3" style="height:40px;">
               			<td align="center" colspan=4>
							<a class="nui-button"  onclick="search()" style="width:60%;">查询</a>
						</td>
               		</tr>
		   		</table>   -->
 			</form>
...
<script type="text/javascript">
	...
	var form = $G.getForm("form1");
  	...
  	function search(){
  		...
  		findBarMapDate(data);
  		var yjdatatype=$G.get("yjdatatype").getValue();//1-是确认问题数量     0-预警数量
  		loadPanoramic(yjdatatype);
  	}
  	   
	function findBarMapDate(data){
  		$G.ajax({
    	  	data:data,
    	  	url:"<%=root%>/panorama/getYjBarList",
  ...
  //前一页
	function back(){
		...
		if(panIndex >= 2){
			...
			data['rwtype']='1';
			$.ajax({
				url: "<%=root%>/panorama/getYjTableList",
				data:data,
				cache: false,
				success: function (text) {
				  ...
				  var yjdatatype=$G.get("yjdatatype").getValue();//1-是确认问题数量     0-预警数量
				  loadHtml(headers,formt,totalPage,panIndex,yjdatatype);
				}
			})
		}else{
			return;
	...
	//加载机构全景视图数据
	function loadPanoramic(yjdatatype){
		var data = form.getData();
		//总页数
		var panPage = $G.get("panPage").getValue();
		//当前页
	...
	function loadHtml(headers1,formt,totalPage,panIndex,yjdatatype){
			var grid = $("#datagrid1");
			var colspan ;
			 var html='';
			if(yjdatatype=='1'){
				colspan = headers1.length;
				html = '<tr class="tr1 ">'+
			       '<td class="td1 border_r" rowspan="2" style="width:10%;background:#2fa2fd;color:aliceblue;" filed="orgname">机构名称</td>'+
	...
	function openDetail(bizParams){
		var url = "<%=root%>/panorama/toYjDetailTable";
