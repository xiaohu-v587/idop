...
<body>
<div id="panel1" class="nui-panel" title="已处理任务" style="width:100%;" showToolbar="false" showCollapseButton="false"
		...
		<table style="width: 100%;">
			<tr>
				<td>
					...
					<a class="nui-button" id="revokeBtn" onclick="getViewSummarData()">查看汇总结果</a>
	...
	<div class="nui-fit">
		<div id="datagrid1" dataField="datas" class="nui-datagrid" style="width: 100%; height: 95%;" url="<%=root%>/missionViewForUser/getList"
			...
			<div property="columns">
				...
				<div field="mission_strat_ymd" width="60px"  allowSort="true" headerAlign="center" align="center"  dateFormat="yyyy-MM-dd ">任务开始时间</div>
				<div field="mission_end_ymd" width="60px" allowSort="true" headerAlign="center" align="center"  dateFormat="yyyy-MM-dd ">任务结束时间</div>
...
<script type="text/javascript">
	...
	function taskImplemSituation(){
		...
		if(row){
			...
			if(row.user_mission_status=="05"){
				...
				$G.open({
					url : "/missionForUser/taskImplementSituation?id="+row.mission_issue_id+"&upMissionId="+row.id+"&type=finshUser_table"+"&missionflag="+row.mission_flag,
					title : titleName,
					width : 1000,
					height : 750,
					onload : function() {
	 ...
	 */
	 function getViewSummarData(){
	    ...
	    if(row){
			var url = "<%=root%>/missionView/viewSummaryData?id="+row.id+"&missionIssueId="+row.mission_issue_id+"&type=user_table"+"&missionflag="+missionflag;
	...
	function download(){
		 ...
		 createfrom["form"] = {action:"<%=root%>/missionViewForUser/downloadTaskList"};
		 downFile(createfrom);
    } 
	 ...
	 function operateRenderer(e){
		 ...
		 var obj = "<a class=\"nui-button\" href='javascript:trackTask(\""+e.row.id+"\",\""+missionIssueId+"\",\""+missionname+"\")' style='text-decoration:none'>任务跟踪</a> ";
		 if(missionflag == 0){
