...
<html>
	...
	<body>
		<div id="panel1" class="nui-panel" title="KPI参数设置" style="width:100%;" showToolbar="false" showCollapseButton="false"
			...
			<div class="nui-fit">
		    	<div id="datagrid1" dataField="data" class="nui-datagrid" style="width:100%;height:100%;" url="<%=root%>/pccm_kpi_param/getList" multiSelect="false">
			        <div property="columns">            
			            <div type="checkcolumn" name="checkCloumn" mupltipart></div>
			            <div field="id" visible="false"></div>
			            <div field="khjl_type" name="khjl_type" headerAlign="center" allowSort="true"  align="center">客户经理类型</div>                
			            <div field="zj" name="zj" allowSort="true" headerAlign="center" align="left">职级</div> 
			            <div allowSort="true" headerAlign="center" align="center">
			            	指标
			            	<div property="columns">
			            		<div field="zbmc_name" allowSort="true" headerAlign="center" align="left">指标名称</div>
			            		<div field="zbmc_val" allowSort="true" headerAlign="center" align="left">指标值</div>
			            	</div>
			            </div>           
<!-- 			            <div field="gs" name="gs" width="150" allowSort="true" headerAlign="center" align="center" >公式</div> -->
			        </div>
				</div>
			</div>
	</body>
...
<script type="text/javascript">
	...
	grid.on("load",function(){
		grid.mergeColumns(["khjl_type","zj","gs"]);
	});
	grid.load();

	//新增
	function add() {
        var url = "<%=root%>/pccm_kpi_param/form";
        var bizParams = {pageType:"add"};
        $G.showmodaldialog("新增", url, 520, 350, bizParams, function(action){
	    	 grid.reload();
	    });
	}
	...
	function edit(){
		...
		if (row) {
	        ...
	        $G.showmodaldialog("编辑", url, 520, 350, bizParams, function(action){
		    	 grid.reload();
		    });
		}else{
			$G.alert("请先选中一条记录！");
		...
		if(rows.length>0){
			$G.GcdsConfirm("确定删除选中记录？", "删除提示", function(action) {
				if (action == 'ok') {
                	...
                	$G.postByAjax({"ids":ids}, "<%=root%>/pccm_kpi_param/del", ajaxConf);
              	}
            });
		}else{
			$G.alert("请选中一条记录");
