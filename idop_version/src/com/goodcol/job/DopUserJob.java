 ...
 *
 */
public class DopUserJob extends OuartzJob {

	@Override
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		try{
		
//			Db.use("gbase").update("CALL ap_idop.DROP_IDOP_SYNC_USER_INFO(@a,@b)");
			Long maxRow =  Db.use("gbase").queryLong("select count(1) from sys_user_info_20190625");
			...
			//不修改字段
			String [] filtercolumns = "role_id,name,phone".split(",");
			
			for (;pageNumber<=maxPage;pageNumber++) {
				
				Page<Record> page =  Db.use("gbase").paginate(pageNumber, pageSize, "select *", "from sys_user_info_20190625");
				
				for (Record r : page.getList()) {
				    Record user = Db.findById("sys_user_info",  r.get("id"));
					if(user != null){
...
//						}
//						
//						Db.update("sys_user_info","id", r);
						//recordList1.add(r);
					}else{
						Db.save("sys_user_info","id", r);
						//recordList2.add(r);
					}
				}
			}
