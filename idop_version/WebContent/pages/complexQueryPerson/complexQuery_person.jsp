...
<form id = "form1" style="border: 1px #f0eff5 solid;width:100%" >
	<table style="width:96%">
            <tr >
              	<td style="padding-left:15px;width:9%">机构名称：</td>
                <td align="left">
						<input id="orgid"  class="nui-treeselect" url="<%=root%>/org/getListByUser" dataField="datas" 
							name="orgid" textfield="orgname" valuefield="id" parentfield="upid"  valueFromSelect="false" 
    					 	expandOnLoad="0" allowInput="false" showClose="true" oncloseclick="onCloseClick" showRadioButton="true" showFolderCheckBox="true"
       						popupWidth="305" popupHeight="250" popupMaxHeight="280" onvaluechanged="" style="width:96%" required="true"/>
								...
								<td class="search_li" style="padding-left:15px">柜员名称：</td>
				                <td align="left" colspan="1" >
									<input id="followed_teller" name="followed_teller" width="96%"  class="mini-lookup"  textField="name"
										valueField="user_no" popupWidth="auto" popup="#gridPanel1" grid="#collarGrid"  value="" text="" 
										onvalidation="" onvaluechanged="" multiSelect="true"/>
								</td>
									<div id="gridPanel1" class="mini-panel" title="header" iconCls="icon-add" style="width: 380px; height: 200px;"
										...
										<div property="toolbar" style="width: 380px; padding: 5px; padding-left: 8px; text-align: center;">
											<div style="float: left; padding-bottom: 2px;">
												<span>员工号或姓名：</span> 
												<input id="keyText" name="keyText" class="mini-textbox" style="width: 120px;"  emptyText="请输入"  onenter="onSearchClick('collarGrid')"  maxlength="20" vtype="maxLength:20" />
            ...
            <td style="padding-left:15px;width:9%">业务模块:</td>
                <td align="left">
                	<input class="mini-combobox" width="96%"  nullItemText="请选择..." emptyText="请选择..."  url="<%=root%>/param/getDict?key=dop_ywtype" 
           	 				id="ywtype" name="ywtype"   onvaluechanged="moduleChanged" valueField="val" textField="remark"></input>
                </td>
               
      
        			<td class="form_label" style="padding-left:15px;width:9%" >业务类型:</td>
	                	<td colspan="1">
        			...
        			<td class="form_label" style="padding-left:15px">指标名称:</td>
	                	<td colspan="1">
	                		<input class="mini-combobox" width="96%"  nullItemText="请选择..."  multiSelect="true" emptyText="请选择..."  url="" 
        	...
        	<td style="padding-left:15px;width:9%">预警类型：</td>
        			<td>	
			...
			<tr >				
       		...
       		<td class="form_label"  style="padding-left:15px" >开始时间:</td>
            <td>
...
<!--    <h:hidden property="query_out" id="query_out"/>
 -->  </form>
    ...
    <div class="mini-fit" >
    	<span id = "info1" style="display:block;font-size:14px;">单人查询</span>
		<div id="datagrid1" dataField="data" class="mini-datagrid" style="width: 100%; height: 97%;" sortMode="client" emptyText="没有对应的记录"
			allowUnselect="false" url="<%=root%>/complexQueryPerson/queryData" multiSelect="true" allowCellSelect="false" allowResize="true" autoEscape="false"
			...
			<div property="columns">
	            ...
	            <div field="deptname"  name="deptno" headerAlign="center"  allowSort="true"  align="center" >机构名称</div>
	            <div field="userno"  name="userno" headerAlign="center"  allowSort="true"  align="center" >人员名称</div>
	            <div header="指标/预警">·
	            	<div property="columns">
        ...
        <span id = "info2" style="display:none;font-size:14px;">多人查询</span>
        <div id="datagrid2" dataField="data" class="mini-datagrid" style="width: 100%; height: 95%; display:none;" sortMode="client" emptyText="没有对应的记录"
			allowUnselect="false" url="<%=root%>/complexQueryPerson/queryData?type=1" multiSelect="true" allowCellSelect="false" allowResize="true" autoEscape="false"
...
function branch_add(e){
	var arr =["ywtype","sub_busi_code","mark_code","warning_type", "warn_name","compareValue"];
...
function branch_query(e){
	...
	//var followed_teller=$G.get("followed_teller").getValue();
	var userid = $G.get("followed_teller").getValue();
	...
	<%-- $.ajax({
 		url : "<%=root%>/complexQueryPerson/judgeCondition",
 		data:{orgid:orgid},
 		async:false,
	...
	var data = {orgid:orgid,followed_teller:userid,begindate:begindate,enddate:enddate,comparevalue:comparevalue,yjstr:yjstr,zbstr:zbstr};
	if(userid.indexOf(",")!=-1){//此时为多人员
		...
		$.ajax({
			url:"<%=root%>/complexQueryPerson/queryData",
			data:data,
			success : function(text) {
		...
		columnControl(comparevalue);
		grid1.on("load",function(){
			grid1.mergeColumns([ "deptno","userno","module_name","ywlx"]);
			$G.unmask();
		})
...
function columnControl(comparevalue){
	...
	if(comparevalue.indexOf('4')=="-1"){
		$G.get("datagrid1").hideColumn(10);
	}else{
		$G.get("datagrid1").showColumn(10);
	}
...
function loadMarkCode(e){
	...
	var url="<%=root%>/complexQueryPerson/getMarkParamList?sub_busi_code="+sub_busi_code;
	$G.getbyName("mark_code").setUrl(url);
}
function warnTypechange(e){
	var warning_type = $G.get("warning_type").getValue();
	$G.get("warn_name").setValue("");
	var url = "<%=root%>/complexQueryPerson/getWarnNameList?val="+warning_type;
...
/* 	if(comparevalue==""){
	...
	var url="<%=root%>/complexQueryPerson/exportRy?followed_teller="+userid+"&begindate="+begindate+"&enddate="+enddate+"&comparevalue="+comparevalue+"&yjstr="+yjstr+"&zbstr="+zbstr;
	window.location=url;
