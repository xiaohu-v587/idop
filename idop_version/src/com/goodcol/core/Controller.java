...
public abstract class Controller {
	...
	public Controller keepPara(Class type, String name) {
		...
		if (values != null) {
			if (values.length == 1)
				try {request.setAttribute(name, TypeConverter.convert(type, values[0]));} catch (ParseException e) {com.goodcol.kit.LogKit.logNothing(e);}
			else
				request.setAttribute(name, values);
		}
		return this;
	...
	public Controller keepModel(Class<? extends com.goodcol.plugin.activerecord.Model> modelClass, String modelName) {
	...
	public Controller keepModel(Class<? extends com.goodcol.plugin.activerecord.Model> modelClass) {
	...
	public void createToken(String tokenName, int secondsOfTimeOut) {
		com.goodcol.token.TokenManager.createToken(this, tokenName, secondsOfTimeOut);
	}
	...
	public boolean validateToken(String tokenName) {
		return com.goodcol.token.TokenManager.validateToken(this, tokenName);
	}
	
	/**
	 * Check token to prevent resubmit  with default token key ---> "GCDS_TOKEN_KEY"
	 * @return true if token is correct
	 */
	...
	/**
	 * Render with view use default type Render configured in GCDSConfig
	 */
	 ...
	 * <p>
	 * Example: renderJson(new User().set("name", "GCDS").set("age", 18));
	 */
	...
	/**
	 * Render with view and status use default type Render configured in GCDSConfig
	 */
	...
	/**
	 * Render with view and errorCode status configured in GCDSConfig
	 */
	...
	public boolean validateCaptcha(String paraName) {
		return com.goodcol.captcha.CaptchaRender.validate(this, getPara(paraName));
	}
