 ...
 * 
 */
public class PccmMarketJob extends OuartzJob {
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		...
		System.out.println("商机转化率定时任务开始：" + DateTimeUtil.getTime());
		...
		try {
			...
			for (Record record : list) {
				...
				int succNum = custKpiServer.findMarketNumByUserNo(user_no,
						"succ");
			...
			//修改定时任务为GBASE数据库中的存储过程  2018年9月8日15:38:06  --liutao
			long jobstartTime = System.currentTimeMillis();
			//获取GBASE数据库宽表数据的最新日期
			
			/*********************更新个人潜在达成率数据******************/
			String validSql = " CALL ap_pccm.DPRO_PCCM_USER_MARKET_INFO(@a,@b)";
			Db.use(GBASE).update(validSql);
			
			/*********************更新机构潜在达成率数据******************/
			validSql = " CALL ap_pccm.DPRO_PCCM_ORG_MARKET_INFO(@a,@b)";
			
			Db.use(GBASE).update(validSql);
			long jobEndTime = System.currentTimeMillis();
			System.out.println("商机转化率定时任务执行结束, 耗时：" + (jobEndTime - jobstartTime) + "毫秒");
			success();
		} catch (Exception e) {
			error();
			e.printStackTrace();
