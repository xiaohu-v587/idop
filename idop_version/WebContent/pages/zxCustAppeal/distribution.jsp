...
<body>    
	<div style="width:100%;height:43%;overflow: hidden">
		...
		<form id="form1" method="post">
        	...
        	<input name="appealid" class="nui-hidden"/>
...
<script type="text/javascript">
	...
	function setData(data){
		...
		var id=infos.id;
		var appealid=infos.appealid;
		$G.getbyName("id").setValue(id);
		$G.getbyName("appealid").setValue(appealid);
	    ...
	    ajaxConf.setSuccessFunc(function (text){
			...
			if(text && text.record && text.record.claim_cust_mgr_name){
				...
				var claimCustMgrId=text.record.claim_cust_mgr_id.split(",");
				var htmlstr="";
				for(var i=0;i<claimCustMgrName.length;i++){
					...
					htmlstr+="<tr id='"+trid+"'> "
							+"	<td align='right'>客户经理：</td> "
							+"	<td align='left'>     "
							+"		<input id='mgrName"+trid+ "' name='mgrName"+trid+ "' value='"+claimCustMgrName[i]+"' onvaluechanged='dataSelCha()' allowInput='false' class='nui-textbox' required='true' style='width:90px;'/> "
							+"		<input id='mgrId' name='mgr"+trid+ "' class='nui-hidden' value='"+claimCustMgrId[i]+"'/> "
							+"		<a class='nui-button' onclick='selRow(\""+trid+ "\")'>选中</a>  "
							+"	</td> "
							+"	<td align='right'>认领比例：</td> "
							+"	<td align='left'>     "
							+"		<input name='prop"+trid+ "' class='nui-textbox' required='true' vtype='int' maxlength='3' value='"+claimProp[i]+"' style='width:90px;' onvaluechanged='dataProCha()' />% ";	
					if(i!=0){
					...
					propAll += parseInt(claimProp[i]);
				}
				$("#tbody").html(htmlstr);
				$G.parse();
				$G.getbyName("mgr_ids").setValue(text.record.claim_cust_mgr_id);
				$G.getbyName("mgr_names").setValue(text.record.claim_cust_mgr_name);
				$G.getbyName("claim_props").setValue(text.record.claim_prop);
				$G.getbyName("appealid").setValue(appealid);
			}
			
		});
		$G.postByAjax({id:infos.id},"<%=root%>/zxCustAppeal/getDetail",ajaxConf);
	}
	...
	function dataProCha(){
		...
		if(propAll&&parseInt(propAll)>100){
			msg= "分配比例之和不能大于100%！";
		}
		msgIsShow(msg);
	}
	...
	function save(){
    	...
    	var urlStr = "<%=root%>/zxCustAppeal/distsave";
	    var ajaxConf = new GcdsAjaxConf();
