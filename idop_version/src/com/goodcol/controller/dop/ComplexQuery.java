 ...
 *
 */
@RouteBind( path = "/complexQuery")
@Before({ManagerPowerInterceptor.class})
public class ComplexQuery extends BaseCtl{
	...
	public void getWarnNameList(){
		...
		String warnWd = getPara("warn_wd");
		String index=getPara("index");
		//warning_code like 'GY902%'
		String sql="";
		if("2".equals(index) || StringUtils.isEmpty(index)){
			 sql="select t.warning_name,t.warning_code from dop_warning_param  t where t.warning_type_code=(select val from dop_gangedmenu_info t where t.id = "+"'"+warnType+"') and (t.is_use is  null or t.is_use = '1') and t.warning_dimension !='2' order by t.warning_name asc";
		}else{
			 sql="select t.warning_name,t.warning_code from dop_warning_param  t where t.warning_type_code=(select val from dop_gangedmenu_info t where t.id = "+"'"+warnType+"') " +
			 		"and (t.is_use is  null or t.is_use = '1') and t.warning_code like '%902%' and t.warning_dimension !='2'  order by t.warning_name asc";
		}
	...
	public void queryData() {
		...
		if(orgid.contains(",")){//此时为多机构
			List<Record> rsList = new ArrayList<Record>();
			
			
			String sql;
			sql = "call ap_idop.DPRO_GROUP_QRY_SINGLE('"+orgid+"','"+begindate+"','"+enddate+"','"+zbstr+"','"+yjstr+"','"+compares.toString()+"','"+userNo+"',@a,@b)";
			Db.use("gbase").find(sql);
			
			String selectstr = "SELECT t.deptno,t.module_name,t.ywlx,t.indexcode,t.indexname,nvl((case when unit ='%' then concat (round(t.value0*100,2),'%') else t.value0  end ),'--') as value0,"+
				     ...
				     "nvl(t.value9,'--') as value9,nvl(t.value10,'--') as value10,nvl(t.value11,'--') as value11,nvl(t.value12,'--') as value12,t.flag,t.czy FROM ap_idop.dop_group_query t left join dop_mark_param dmp on t.indexcode=dmp.mark_code  where t.czy = ? order by t.module_name";
			...
			Record orgr = new Record();
			for (Record org : orgList) {
				orgr.set(org.getStr("orgnum"), org.get("orgname"));
			}
				...
				String deptno,module_name,ywlx,indexname;
				
				
				for (Record r : rList) {
					 deptno = r.getStr("deptno");	
					 module_name = r.getStr("module_name");
					 ywlx = r.getStr("ywlx");
					 indexname = r.getStr("indexname");
					 //初始化业务模型参数
					 reset(modelMap, deptno, module_name, ywlx, indexname, r);
				}
				...
				for (String onekey : onekeys) {//业务模型
				    ...
				    for (String twokey : twokeys) {//业务类型
						...
						for (String threekey : threekeys) {//组合参数
				    		...
				    		Map<String,List<Record>> deptmap = modelMap.get(onekey).get(twokey).get(threekey);
				    		Set<String> deptmapkeys = deptmap.keySet();
				    		cloumnIndex++;
				    		//类型名称
				    		...
				    		//按照机构组装数据行
				    		for (String deptmapkey : deptmapkeys) {
				    			List<Record> tLsit = deptmap.get(deptmapkey);
				    			
				    			if(!dataMap.containsKey(deptmapkey)){
				    				dataMap.put(deptmapkey, new Record());
				    				dataMap.get(deptmapkey).set("deptno",deptmapkey);
				    				dataMap.get(deptmapkey).set("deptname",orgr.get(deptmapkey));//进行机构中文转换
				    				dataMap.get(deptmapkey).set("begindate",begindate);
				    				dataMap.get(deptmapkey).set("enddate",enddate);
				    			}
			...
			List<Record> rList = Db.use("gbase").find("select deptno,module_name,ywlx,indexcode,indexname,"
					...
					"value5,value6,value7,value8,value9,value10,value11,value12,flag,czy from ap_idop.dop_group_query dki left join dop_mark_param dmp on dki.indexcode=dmp.mark_code where czy = ? order by deptno,module_name,ywlx desc",userNo);
			...
			for (Record record : rList) {
				record.set("deptno",orgr.get(record.getStr("deptno")));
			}
		...
		// 记录日志
		LoggerUtil.getIntanceof().saveLogger(getCurrentUser().getStr("USER_NO"), "查询报表", "9", "组合-查询");
		log.info("组合-查询");
		// 返回json数据
		renderJson();
	}
	
	public boolean reset(Map<String,Map<String,Map<String,Map<String,List<Record>>>>> modelMap,String deptno,String module_name,String ywlx,String indexname,Record r){
		boolean flag = false;
		if(modelMap.containsKey(module_name)){
			 if(modelMap.get(module_name).containsKey(ywlx)){
				 if(modelMap.get(module_name).get(ywlx).containsKey(indexname)){
					 if(modelMap.get(module_name).get(ywlx).get(indexname).containsKey(deptno)){
						 modelMap.get(module_name).get(ywlx).get(indexname).get(deptno).add(r);
					 }else{
						 modelMap.get(module_name).get(ywlx).get(indexname).put(deptno, new ArrayList<Record>());
						 modelMap.get(module_name).get(ywlx).get(indexname).get(deptno).add(r);
					 }
				 }else{
		...
		if(flag){
			return reset(modelMap, deptno, module_name, ywlx, indexname, r);
		}else{
			return flag;
		}
	...
	public void exportJg(){
		...
		if(orgid.contains(",")){//多维度
			List<Record> rsList = new ArrayList<Record>();
			/**********导出二级维度的数据***********/
			
			
			log.info("单机构查询导出二级维度机构数据存储过程 ap_idop.DPRO_GROUP_QRY_SINGLE 调用开始:"+DateTimeUtil.getTime());
			sql = "call ap_idop.DPRO_GROUP_QRY_SINGLE('"+orgid+"','"+begindate+"','"+enddate+"','"+zbstr+"','"+yjstr+"','"+compares.toString()+"','"+userNo+"',@a,@b)";
			Db.use("gbase").update(sql);
			System.out.println("二级维度机构数据计算SQL：" + sql);
			log.info("单机构查询导出二级维度机构数据存储过程 ap_idop.DPRO_GROUP_QRY_SINGLE  调用结束:"+ DateTimeUtil.getTime()); 
			
			
			//查询
			List<Record> SList = Db.use("gbase").find("select * from ap_idop.dop_group_query where czy = ? order by module_name",userNo);
	/*		for(Record SsList :SList){
				if(SsList.get("value0") !=null){
					if(SsList.get("value0").toString().contains(".0000"))
					SsList.set("value0", SsList.get("value0").toString().substring(0, SsList.get("value0").toString().length()-5));
				}
				if(SsList.get("value1") !=null){
					if(SsList.get("value1").toString().contains(".0000"))
					SsList.set("value1", SsList.get("value1").toString().substring(0, SsList.get("value1").toString().length()-5));
				}
				if(SsList.get("value2") !=null){
					if(SsList.get("value2").toString().contains(".0000"))
					SsList.set("value2", SsList.get("value2").toString().substring(0, SsList.get("value2").toString().length()-5));
				}
				if(SsList.get("value3") !=null){
					if(SsList.get("value3").toString().contains(".0000"))
					SsList.set("value3", SsList.get("value3").toString().substring(0, SsList.get("value3").toString().length()-5));
				}
			...
			log.info("单机构查询导出一级维度机构数据存储过程 ap_idop.DPRO_GROUP_QRY_SINGLE 调用开始:"+DateTimeUtil.getTime());
			sql = "call ap_idop.DPRO_GROUP_QRY_SINGLE('"+orgid+"','"+begindate+"','"+enddate+"','"+zbstr+"','"+yjstr+"','"+compares.toString()+"','"+userNo+"',@a,@b)";
			Db.use("gbase").update(sql);
			System.out.println("一 级维度机构数据计算SQL：" + sql);
			log.info("单机构查询导出一级维度机构数据存储过程 ap_idop.DPRO_GROUP_QRY_SINGLE  调用结束:"
					+ DateTimeUtil.getTime()); 
			//exportMxfExcel(userNo,compareValue);
			exportExcel(orgid,userNo,compareValue);
		}
		renderNull();
	}
	//导出多维度 ()
	public void exportMxsExcel(String userNo,List<Record> rsList,List<Record> threeColumns,Set<String> onekeys,Set<String> twokeys){
        ...
        String[] onek=new String[lens+3];
        String[] twok=new String[lens+3];
        onek[0]="机构";
        onek[1]="开始时间";
        onek[2]="结束时间";
        twok[0]="机构";
		...
		String [] headers1 = new String[threeColumns.size()+3];
		String [] columns1 = new String[threeColumns.size()+3];
		headers1[0]="deptname";
		columns1[0]="机构名称";
		headers1[1]="begindate";
		columns1[1]="开始时间";
		headers1[2]="enddate";
		columns1[2]="结束时间";
		for(int i=0;i<threeColumns.size();i++){
			headers1[i+3]=threeColumns.get(i).getStr("field");
			columns1[i+3]=threeColumns.get(i).getStr("header");
		}
		 arr[2]=columns1;
		 
		//设置 单元格 合并位置起始行，结束行，起始列，结束列
			String [] oneMulNum  = {"0,2,0,0","0,2,1,1","0,2,2,2","0,0,3,"+(threeColumns.size()+2),"1,1,3,"+(threeColumns.size()+2)};
		 ...
		 ExcelRenderx er = null;
			String fileName = "";
			try {
				fileName = new String("组合查询数据展现.xls".getBytes("GB2312"), "ISO_8859_1");
			} catch (UnsupportedEncodingException e) {
	...
	/*	ExcelRender er = null;
		...
		er = new ExcelRender(fileName, columns1, headers1, rsList, getResponse());
		er.render(); */
	...
	//单维度
	public void exportExcel(String orgid, String userNo, String compareValue){
		//查询
		List<Record> rlist = Db.use("gbase").find("select deptno,module_name,ywlx,indexname ,( case when flag='0' then '指标' else '预警' end ) flags" +
				", value0,value1,value2,value3 from ap_idop.dop_group_query where czy = ?",userNo);
		
		
		//机构查询
		String org=Db.queryStr("select orgname from sys_org_info where orgnum= ?", new Object[] { orgid });
...
/**************************对应的数据与表头一致*********************************/
		...
		oneMulNum[0]="0,1,0,0";
		oneMulNum[1]="0,0,1,3";
		for(int kk=2;kk<list.size()-2;kk++){
	...
	//导出单维度 (表头未合并，数据也未合并)
/*	
	public void exportMxfExcel(String userNo,String compareValue){
		//查询
		List<Record> list = Db.use("gbase").find("select deptno,module_name,ywlx,indexname ,( case when flag='0' then '指标' else '预警' end ) flags" +
				", value0,value1,value2,value3 from ap_idop.dop_group_query where czy = ?",userNo);
		for(Record SsList :list){
			if(SsList.get("value0") !=null){
				if(SsList.get("value0").toString().contains(".0000"))
				SsList.set("value0", SsList.get("value0").toString().substring(0, SsList.get("value0").toString().length()-5));
			}
		...
		//************************compareValue选择单个的时候header********************************//*
		// 指定用哪些查询出来的字段填充excel文件
       //compareValue=1的时候
		String[] header1s={ "deptno","module_name","ywlx", "indexname","flags","value0","value1"};
		//compareValue=2的时候
		String[] header2s={ "deptno","module_name","ywlx", "indexname","flags","value0","value2"};
		//compareValue=3的时候
		String[] header3s={ "deptno","module_name","ywlx", "indexname","flags","value0","value3"};
		//compareValue=4的时候;
		String[] header4s ={ "deptno","module_name","ywlx", "indexname","flags","value0"};
		//************************compareValue选择多个的时候header********************************//*
		//compareValue=1，2的时
		String[] header12s={ "deptno","module_name","ywlx", "indexname","flags","value0","value1","value2"};
		//compareValue=1，3的时
		String[] header13s={ "deptno","module_name","ywlx", "indexname","flags","value0","value1","value3"};
		//compareValue=2，3的时
		String[] header23s={ "deptno","module_name","ywlx", "indexname","flags","value0","value2","value3"};
		//compareValue=1,2，3的时
		String[] header123s={ "deptno","module_name","ywlx", "indexname","flags","value0","value1","value2","value3"};
		// excel文件的每列的名称,顺序与填充字段的顺序保持一致
		//************************compareValue选择单个的时候column1s********************************//*
		//compareValue=1(上日)的时候;
        String[] column1s = { "机构名称", "业务模块","业务类型", "名称","类型","基准值","上日比较值" };	
        //compareValue=2(上月末)的时候;
        String[] column2s = { "机构名称", "业务模块","业务类型", "名称","类型","基准值","上月末比较值" };	
        //compareValue=3(上年末)的时候;
        String[] column3s = { "机构名称", "业务模块","业务类型", "名称","类型","基准值","上年末比较值" };	
        //compareValue=4(同比)的时候;(其他组合包含 14,24,34,1234)
        String[] column4s = { "机构名称", "业务模块","业务类型", "名称","类型","基准值"};	
        //************************compareValue选择多个的时候column1s********************************//*
		//compareValue=1,2(上日,)的时候;
        String[] column12s = { "机构名称", "业务模块","业务类型", "名称","类型","基准值","上日比较值","上月末比较值" };	
        //compareValue=1,3(上日,上月末)的时候;
        String[] column13s = { "机构名称", "业务模块","业务类型", "名称","类型","基准值","上日比较值", "上年末比较值" };	
        //compareValue=2,3(上日,上年末)的时候;
        String[] column23s = { "机构名称", "业务模块","业务类型", "名称","类型","基准值","上月末比较值","上年末比较值" };	
        //compareValue=123(上日,上月末,上年末)的时候;
        String[] column123s = { "机构名称", "业务模块","业务类型", "名称","类型","基准值","上日比较值","上月末比较值","上年末比较值" };
		...
		if(compareValue.contains(",")){
			String[] cv=compareValue.split(",");
			if(cv.length==2){
				////设置 单元格 合并位置起始行，结束行，起始列，结束列
				String [] oneMulNum2  = {"0,1,0,0","0,0,1,3","0,1,4,4","0,1,5,5","0,1,6,6","0,1,7,7"};
				columnsMulNum[0] = oneMulNum2;
				if(cv[0].equals("1")&& cv[1].equals("2")){
					oneRowArr[6]="上日比较值";
					oneRowArr[7]="上月末比较值";
					header2[0]=oneRowArr;
					header2[1]=column12s;
					er = new ExcelRenderx(fileName, header12s, header2,columnsMulNum, list, getResponse());
					//ex=new ExcelRender(fileName, column12s, header12s, list, getResponse());
				}else if(cv[0].equals("1")&& cv[1].equals("3")){
					oneRowArr[6]="上日比较值";
					oneRowArr[7]="上年末比较值";
					header2[0]=oneRowArr;
					header2[1]=column13s;
					er = new ExcelRenderx(fileName, header13s, header2,columnsMulNum, list, getResponse());
					//ex=new ExcelRender(fileName, column13s, header13s, list, getResponse());
				}else if(cv[0].equals("2")&& cv[1].equals("3")){
					oneRowArr[6]="上月末比较值";
					oneRowArr[7]="上年末比较值";
					header2[0]=oneRowArr;
					header2[1]=column23s;
					er = new ExcelRenderx(fileName, header23s, header2,columnsMulNum, list, getResponse());
					//ex=new ExcelRender(fileName, column23s, header23s, list, getResponse());
				}else{
					////设置 单元格 合并位置起始行，结束行，起始列，结束列
					String [] oneMulNum2s  = {"0,1,0,0","0,0,1,3","0,1,4,4","0,1,5,5","0,1,6,6"};
					columnsMulNum[0] = oneMulNum2s;
					if(cv[0].equals("1")&& cv[1].equals("4")){
						oneRowArr[6]="上日比较值";
						header2[0]=oneRowArr;
						header2[1]=column1s;
						er = new ExcelRenderx(fileName, header1s, header2,columnsMulNum, list, getResponse());
						//ex=new ExcelRender(fileName, column1s, header1s, list, getResponse());
					}else if(cv[0].equals("2")&& cv[1].equals("4")){
						oneRowArr[6]="上月末比较值";
						header2[0]=oneRowArr;
						header2[1]=column2s;
						er = new ExcelRenderx(fileName, header2s, header2,columnsMulNum, list, getResponse());
						//ex=new ExcelRender(fileName, column2s, header2s, list, getResponse());
					}else if(cv[0].equals("3")&& cv[1].equals("4")){
						oneRowArr[6]="上年末比较值";
						header2[0]=oneRowArr;
						header2[1]=column3s;
						er = new ExcelRenderx(fileName, header3s, header2,columnsMulNum, list, getResponse());
						//ex=new ExcelRender(fileName, column3s, header3s, list, getResponse());
					}
				} 
			}else if(cv.length==3){
				////设置 单元格 合并位置起始行，结束行，起始列，结束列
				String [] oneMulNum3  = {"0,1,0,0","0,0,1,3","0,1,4,4","0,1,5,5","0,1,6,6","0,1,7,7","0,1,8,8"};
				columnsMulNum[0] = oneMulNum3;
				if(cv[0].equals("1")&& cv[1].equals("2")&& cv[2].equals("3")){
					oneRowArr[6]="上日比较值";
					oneRowArr[7]="上月末比较值";
					oneRowArr[8]="上年末比较值";
					header2[0]=oneRowArr;
					header2[1]=column123s;
					er = new ExcelRenderx(fileName, header123s, header2,columnsMulNum, list, getResponse());
					//ex=new ExcelRender(fileName, column123s, header123s, list, getResponse());
				}else {
					////设置 单元格 合并位置起始行，结束行，起始列，结束列
					String [] oneMulNum3s  = {"0,1,0,0","0,0,1,3","0,1,4,4","0,1,5,5","0,1,6,6","0,1,7,7"};
					columnsMulNum[0] = oneMulNum3s;
					if(cv[0].equals("1")&& cv[1].equals("2")&& cv[2].equals("4")){
						oneRowArr[6]="上日比较值";
						oneRowArr[7]="上年末比较值";
						header2[0]=oneRowArr;
						header2[1]=column12s;
						er = new ExcelRenderx(fileName, header12s, header2,columnsMulNum, list, getResponse());
						//ex=new ExcelRender(fileName, column12s, header12s, list, getResponse());
					}else if(cv[0].equals("1")&& cv[1].equals("3")&& cv[2].equals("4")){
						oneRowArr[6]="上日比较值";
						oneRowArr[7]="上年末比较值";
						header2[0]=oneRowArr;
						header2[1]=column13s;
						er = new ExcelRenderx(fileName, header13s, header2,columnsMulNum, list, getResponse());
						//ex=new ExcelRender(fileName, column13s, header13s, list, getResponse());
					}else if(cv[0].equals("2")&& cv[1].equals("3")&& cv[2].equals("4")){
						oneRowArr[6]="上月末比较值";
						oneRowArr[7]="上年末比较值";
						header2[0]=oneRowArr;
						header2[1]=column23s;
			...
			////设置 单元格 合并位置起始行，结束行，起始列，结束列
				String [] oneMulNum4  = {"0,1,0,0","0,0,1,3","0,1,4,4","0,1,5,5","0,1,6,6","0,1,7,7","0,1,8,8"};
				oneRowArr[6]="上日比较值";
				oneRowArr[7]="上年末比较值";
				oneRowArr[8]="上年末比较值";
				header2[0]=oneRowArr;
				columnsMulNum[0] = oneMulNum4;
				header2[1]=column123s;
				er = new ExcelRenderx(fileName, header123s, header2,columnsMulNum, list, getResponse());
				//ex=new ExcelRender(fileName, column123s, header123s, list, getResponse());
			}
		...
		////设置 单元格 合并位置起始行，结束行，起始列，结束列
			String [] oneMulNum1  = {"0,1,0,0","0,0,1,3","0,1,4,4","0,1,5,5","0,1,6,6"};
			columnsMulNum[0] = oneMulNum1;
			...
			er = new ExcelRenderx(fileName, header1s, header2,columnsMulNum, list, getResponse());
				...
				String [] oneMulNum1s  = {"0,1,0,0","0,0,1,3","0,1,4,4","0,1,5,5"};
				columnsMulNum[0] = oneMulNum1s;
				header2[0]=oneRowArr;
				header2[1]=column4s;
				er = new ExcelRenderx(fileName, header4s, header2,columnsMulNum, list, getResponse());
				//ex=new ExcelRender(fileName, column4s, header4s, list, getResponse());
			}
		}
		er.renderMulRow(); 
		//ex.render();
