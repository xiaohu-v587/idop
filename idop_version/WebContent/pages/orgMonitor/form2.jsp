...
<body>
     ...
     <div align="center" style="font-size: 16px;font-weight:bold;height:10%" ></div>
	    <div class="mini-fit">
	    	<div id="datagrid" dataField="data" class="mini-datagrid" style="width: 100%; height: 98%;" sortMode="client" emptyText="没有对应的记录"
				allowUnselect="false" url="<%=root%>/orgMonitor/getDetail" multiSelect="true" allowCellSelect="false"  autoEscape="false"
				  showEmptyText="true" allowSortColumn="true">
				<div id="detailColumns" property="columns"  > 
 		            ...
 		            <div field="org_no" headerAlign="center" width="100"  allowSort="true"  align="center" >机构号</div>  
 		            ...
 		            <div field="count" headerAlign="center" width="100"  allowSort="true"  align="center" dataType="int" >数量</div> 
		        </div>
	       </div>
	    </div>
    </div>
	...
	//获取父页面传递来的json数据
	  function setData(data) {
		  ...
		  var monitor_date=datas.monitor_date;
		  var monitor_name=datas.monitor_name;
	  
		  var columns  =  grid.getColumns();			
			if(flag==1){
				 $.ajax({
	                 url: "<%=root%>/orgMonitor/queryField",	//查询该指标下面需要转码的field
	  		         type: 'post',
	  ...
	  function download(){		
			...
			window.location="<%=root%>/orgMonitor/download?"+connUrlByJson(datas);
		}
