var xlsxUtils = {
     ...
     * @return {Object}
     */
    readDataHead:function(data) {
        var o = {}, d = Array.isArray(data) ? Object.keys(data[0]) : data; for (var i in d) o[i] = i;
        return o;
    },
    /**
     * @desc 格式化数据为Sheet格式
     ...
     */
    format2Sheet:function(json, n, r, keyMap, t) {
        keyMap = keyMap || Object.keys(json[0]);
        var types = (t == undefined ? (function(v) { (({ "number": "n", undefined: "s", "boolean": "b","string":"s" })[typeof v]) } ||"s") : t);
        n = n || 0;
        r = r || 0;
        var tmpdata = {};//用来保存转换好的json 
        var t1 = json.map( function(v, i) { keyMap.map(function(k, j) { Object.assign({}, {
            v: v[k],
            position: ((j + n) > 25 ? xlsxUtils.getCharCol((j + n)) : String.fromCharCode(65 + (j + n))) + (i + 1 + r),
        })})}
        ).reduce( function(prev, next) { prev.concat(next) }).forEach(function(v, i) { tmpdata[v.position] = {
            v: v.v,
            t: types?types(v.v):"s"
        	}
        });
     ...
     * @param {Object} ref
     */
    format2WB:function(sheetData, title, wb, ref) {
        title = title || "mySheet";
