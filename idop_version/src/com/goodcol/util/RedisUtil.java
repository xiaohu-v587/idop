...
/**
 * @author wangfq
 * @version 2017年9月15日 上午9:20:46
 */
public class RedisUtil {
	private static JedisPool pool = null;

	/**
	 * 初始化构建连接池，参数配置在config.properties文件中
	 * 
	 * @return
	 */
	static {
		try {
			...
			// 如果pool已经分配了setMaxTotal个jedis实例，则此时pool的状态为exhausted(耗尽)。
			// 根据配置文件获取，配置文件未配置的话返回默认值
			config.setMaxTotal(PropertiesContent.getToInteger("redis.maxTotal",
					500));
			// 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例。
			config.setMaxIdle(PropertiesContent.getToInteger(
					"redis.maxMaxIdle", 5));
			// 表示当borrow(引入)一个jedis实例时，最大的等待时间，如果超过等待时间，则直接抛出JedisConnectionException；
			config.setMaxWaitMillis(PropertiesContent.getToLong(
					"redis.maxWaitMillis", 100000L));
			// 在borrow一个jedis实例时，是否提前进行validate操作；如果为true，则得到的jedis实例均是可用的；
			config.setTestOnBorrow(PropertiesContent.getToBool(
					"redis.testOnBorrow", true));
			String host = PropertiesContent.get("redis.host");
			int port = PropertiesContent.getToInteger("redis.port", 6379);
			int timeout = PropertiesContent.getToInteger("redis.timeout",
					100000);
	...
	/**
	 * redis缓存是否开启 1、redis连接池未配置成功2、字典项redis开关未打开
	 * 
	 * @param key
	...
	public static boolean isRedisOpen() {
		...
		String kg = ParamContainer.getDictName("SESSION", "1");
		...
		if("open".equals(kg)){
			...
			try {
				...
				System.out.println("redis缓存服务器未配置成功！");
				return false;
			} finally {
	...
	/**
	 * 向redis存入key和value以及过期时间seconds,并释放连接资源 如果key已经存在 则覆盖
	 * 
	 * @param key
	 * @param value
	 * @return 成功 返回OK 失败返回 0
	...
	public static String setObject(String key, Object object,int seconds) {
		...
		try {
			jedis = pool.getResource();
			return jedis.setex(key.getBytes(), seconds, serialize(object));
		} catch (Exception e) {
			returnBrokenResource(pool, jedis);
			e.printStackTrace();
			return "0";
	...
	/**
	 * 通过key获取储存在redis中的value，返回object 并释放连接
	 * 
	 * @param key
	 * @param value
	 * @return 成功 返回OK 失败返回 0
	 ...
	 */
	public static Long appendValue(String key, String str) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = pool.getResource();
	 ...
	 */
	public static String setKeyTimeOut(String key, String value, int seconds) {
		Jedis jedis = null;
		String res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * 
	 */
	public static String setKeys(String... keysvalues) {
		Jedis jedis = null;
		String res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return 成功返回1 失败返回0
	 */
	public static Long setKeysnx(String... keysvalues) {
		Jedis jedis = null;
		Long res = 0L;
		try {
			jedis = pool.getResource();
	 ...
	 * @return 如果没有返回null
	 */
	public static String getRangeValue(String key, int startOffset,
			int endOffset) {
		Jedis jedis = null;
		String res = null;
		try {
	 ...
	 * @return
	 */
	public static Long incrValue(String key, Long integer) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return
	 */
	public static Long decrValue(String key, Long integer) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return 没有返回null
	 */
	public static String getValueByKeyField(String key, String field) {
		Jedis jedis = null;
		String res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return
	 */
	public static Set<String> getValuesSet(String key) {
		Jedis jedis = null;
		Set<String> res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return 添加成功的个数
	 */
	public static Long addSetValue(String key, String... members) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return 删除的个数
	 */
	public static Long removeSetValue(String key, String... members) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return
	 */
	public static Set<String> getValueByScore(String key, String max, String min) {
		Jedis jedis = null;
		Set<String> res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return
	 */
	public static Set<String> getValueByScore(String key, double max, double min) {
		Jedis jedis = null;
		Set<String> res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return
	 */
	public static Long getValueCount(String key, String min, String max) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return
	 */
	public static Long getValueCount(String key) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return
	 */
	public static Long delValueByRank(String key, long start, long end) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = pool.getResource();
	 ...
	 * @return
	 */
	public static Long delValueByScore(String key, double start, double end) {
		Jedis jedis = null;
		Long res = null;
		try {
			jedis = pool.getResource();
