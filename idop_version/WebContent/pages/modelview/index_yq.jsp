...
.td2{width:30px;text-align:center;border-bottom:2px #f0eff5 solid;background-color:#fbfbfb;}

.table_box{margin:0 auto;padding-top:10px;}
...
.page_box{font-size:12px;margin-top:20px;margin-right:1%;width:350px;float:right;}
...
<body>
	<div class="main_box" style="overflow:auto;">
				...
				<form id="form1">
			   		...
			   		<div class="table_1"  style="width:100%;color:#201f35;">
						...
						<ul>
							...
							<li align="left">
								<input name="querytype" id="querytype" class="nui-dictcombobox" textfield="dictName" valuefield="dictID" dictTypeId="dop_querytype" value="1" onvaluechanged="onDopQuerytype"  allowInput="false" />
							</li>
						</ul>
						<ul>
	               			<li align="right" class="search_li">图表格式：</li>
							<li align="left">
								<input name="pattern" id="pattern" class="nui-dictcombobox" textfield="dictName" valuefield="dictID" dictTypeId="dop_pattern1" value="2" onvaluechanged="onDopPattern" />
							</li>
						</ul>
						<ul style="width:35%;">
							<li align="right">
        ...
        <div class="content_box content_c margin_t1" style="width:100%;height:450px">
        	...
        	<div class="content_c" id="mainMap2">
        		...
        		<table cellspacing="0" class="table_box" id="datagrid" style="">
                </table>
                </div>
                 <div  class="content_p">
	                 <ul class="page_box">
...
<script type="text/javascript">
	$G.parse();
	$G.getbyName("querytype").setEnabled(false);
	var root = "<%=root%>";
	var mychart; 
  ...
  /* 	var date = new Date();
  	...
  	function findBaseDate(data,callback){
  		$G.ajax({
    	  	data:data,
    	  	url:"<%=root%>/modelview/getGegendData",
	...
	function findLineMapDate(data,callback){
		data["cloumntype"] = showLineData["cloumntype"];
  		$G.ajax({
    	  	data:data,
    	  	url:"<%=root%>/modelview/getLineList",
    	  	success:function(data){
  ...
  //前一页
	function back(){
		...
		if(panIndex >= 2){
			...
			$.ajax({
				url: "<%=root%>/modelview/getTableList",
				data:data,
				cache: false,
				success: function (text) {
	...
	function loadAreaBasic(){
		...
		$.ajax({
			url: "<%=root%>/modelview/getAreaBasic",
			data:form.getData(),
			cache: false,
			success: function (text) {
			  var data = mini.decode(text);
