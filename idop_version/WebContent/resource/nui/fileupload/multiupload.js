...
mini.extend(UserControl.MultiUpload, mini.DataGrid, {
    ...
    limitSize: "15MB",
    //limitType: "*",
    limitType: "*.jpg;*.JPG;*.jpeg;*.JPEG;*.png;*.PNG;*.bmp;*.BMP;*.pdf;*.PDF;*.doc;*.DOC;*.docx;*.DOCX;*.xls;*.XLS;*.xlsx;*.XLSX",
    ebavleType:false,//默认不启用类型选择，如果为true时，上传类型必须选择文件类型
    ebavleUrl:'',//在ebavleType为true时配置获取字典类型参数
    uploadLimit: 0,
    queueLimit: 10,
    postParam: null,
    ...
    _create: function () {
        ...
        var defaultColumns = [
                    { "type": "indexColumn" },
                    { field: "name", width: 130, header: me.columnsTexts.nameColumnHeader },
                    { field: "pidtype", width: 80, header: me.columnsTexts.actionColumnHeader, align: "center", headerAlign: "center", visible:me.ebavleType },
                    ...
                    { field: "action", width: 40, header: me.columnsTexts.actionColumnHeader, align: "center", headerAlign: "center" }
                  ];
        me.set({
            showPager: false,
            showToolbar: true,
        ...
        sb.push('</td><td><div id="pidtype" style="width:100px;height:25px;" name="pidtype" class="mini-combobox" textField="REMARK" valueField="VAL"  visible="'+me.ebavleType+'"  allowInput="false" showNullItem="true" nullItemText="请选择..."  /> ');
        sb.push('</td><td><a class="mini-button" iconCls="icon-upload" name="multiupload">批量上传</a>');
        sb.push('</td><td><a class="mini-button" iconCls="icon-remove" name="removeAll">清空列表</a>');
        sb.push('</td></tr></table>');
    ...
    bindEvents: function () {
        ...
        if(me.ebavleType == "true"){
        	mini.get(me.id).showColumn(2);
        	mini.get("pidtype").show();
          	me.pidtypeCombox = mini.getbyName("pidtype", toolbarEl);
          	me.pidtypeCombox.url=me.ebavleUrl;
          	mini.getbyName("pidtype").load(me.ebavleUrl);
        }
        ...
        me.on("drawcell", function (e) {
            ...
            if (field == "pidtype") {
            	 if(me.ebavleType){
            		var data = mini.getbyName("pidtype").data
                 	var j = 0;
                 	for(var i=0;i<data.length;i++){
                 		if(value != "" ){
                 			if(data[i].VAL == value){
                     			e.cellHtml = data[i].REMARK;
                     			j=1;
                     		}
                 		}
                 	}
            ...
            if (field == "action") {
                ...
                e.cellHtml += '<a class="upicon-query" name="' + uid + '"><a>';*/

            }
        })
        //$(document.body).on("click", ".upicon-remove", function () {
        /*$(".upicon-remove").bind("click", function(){
       ...
       /* $(document.body).on("click", ".upicon-query", function () {
            var uid = $(this).attr("name");
            var row = me.getRowByUID(uid);
            if (me.uploader.getStats().files_queued !== 0) {
                me.uploader.cancelUpload(row.fileId);
    ...
    __on_file_queued: function (file) {
        ...
        row.name = file.name;
        row.type = file.type;
        row.pidtype=mini.getbyName("pidtype").getValue();
        row.status = 0;
        row.fileId = file.id;
        ...
        me.postParam={pidtype:row.pidtype};
        if (me.autoUpload) {
    ...
    __on_file_queued_error: function (file, errorCode, msg) {
    	...
    	mini.alert("文件选择出错!文件不能大于"+this.limitSize+"！");
    },
    ...
    setEbavleType: function (bol) {
        this.ebavleType = bol;
    },
    getEbavleType: function () {
        return this.ebavleType;
    },
    setEbavleUrl: function (url) {
        this.ebavleUrl = url;
    },
    getEbavleUrl: function () {
        return this.ebavleUrl;
    },
    getAttrs: function (el) {
        var attrs = UserControl.MultiUpload.superclass.getAttrs.call(this, el);
        mini._ParseString(el, attrs,
            ["uploadUrl", "flashUrl", "limitType", "limitSize", "uploadName", "queueLimit", "onuploaderror", "onuploadsuccess","ebavleType","ebavleUrl"]
        );
        mini._ParseBool(el, attrs, ["autoUpload"]
        );
        return attrs;
    }
