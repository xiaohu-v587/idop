...
<body>
          		...
          		<div id="form1" class="mini-form" align="left">
            		<table id="table1" class="table">
              			<tr>
		                   	...
		                   	<td align="left">
								<input id="followed_org" required="true" width="170px;" class="nui-treeselect" url="<%=root%>/org/getListByUser" dataField="datas" 
			                ...
			                <td colspan="1">
			                	<input class="mini-combobox"  width="170px;" required="true"  nullItemText="请选择..." emptyText="请选择..."  url="<%=root%>/param/gettxDict?key=dop_ywtype" 
			            ...
			            <tr>
			                ...
			                <td colspan="1">
			                	<input class="mini-combobox" width="170px;"  nullItemText="请选择..." emptyText="请选择..."  url="<%=root%>/param/gettxDict?key=dop_follow_type" 
    	...
    	<div showCollapseButton="false" style="border: 0;">
      	...
      	<div class="mini-fit">
			<div id="datagrid1" dataField="data" class="mini-datagrid" style="width: 100%; height: 98%;" sortMode="client" emptyText="没有对应的记录"
				allowUnselect="false" url="<%=root%>/myfocus_index/getList" multiSelect="true" allowCellSelect="false"  autoEscape="false"
				  showEmptyText="true">
				<div property="columns">
		            <div type="checkcolumn" ></div>
  ...
  <script type="text/javascript">
    ...
    var bus="<%=request.getAttribute("model")%>";
  	if(bus ==null || bus =="" ||  bus =="null" ){
  		mini.alert("请先选择模块");
  		$G.get("follow_type").setReadOnly(true);
  	    $G.get("followed_org").setReadOnly(true);
  	    $G.get("busi_module").setReadOnly(true);
  	    $G.get("add").setReadOnly(true);
	    $G.get("remove").setReadOnly(true);
  	}else{
  		$G.get("follow_type").setReadOnly(false);
  	    $G.get("followed_org").setReadOnly(false);
  	    $G.get("busi_module").setReadOnly(false);
  	    $G.get("add").setReadOnly(false);
	    $G.get("remove").setReadOnly(false);
  	}
		...
		var rows = grid.getSelecteds();
			...
			mini.confirm("确定删除该关注？", "确定？", function(action) {
				if (action == "ok") {	
					...
					$.ajax({
						url : "<%=root%>/myfocus/delete?key=" + id,
