...
<script type="text/javascript">
	...
	function findJsMap(){
		...
		mychart.on('click', function (params) {
			if(city[params.name]){
				//window.location.href="<%=root%>/panorama/indexYj?orgnum="+city[params.name]+"stat=0";
				parent.createChialedTab({name:params.name,url:"<%=root%>/panorama/indexYj?orgnum="+city[params.name]+"&stat=0",title:params.name+"预警信息"})
			}
		});       
	...
	function back(){
		...
		if(panIndex >= 2){
			...
			$.ajax({
				url: "<%=root%>/panorama/getListByOrg",
	...
	function loadHtml(formt,totalPage,panIndex){
		  ...
		  if(formt != null &&formt.length > 0){
			  for(var i =0;i<formt.length;i++){
				  htmls += "<tr class='tr2'><td class='td2 border_l' align='center' ><a  href='javascript:openParentTab(\""+formt[i].orgname+"运营全景图\",\"<%=root%>/panorama/index?orgnum="+formt[i].deptno+"\")' style='text-decoration:none;color:#000000;' >"+formt[i].orgname+"</a></td><td class='td2 border_r' align='center'><a  href='javascript:openParentTab(\""+formt[i].orgname+"运营全景图\",\"<%=root%>/panorama/index?orgnum="+formt[i].deptno+"\")' style='text-decoration:none;color:#000000;' >"+formt[i].total+"</a></td></tr>";
			  }
			  ...
			  if(formt != null && formt.length > 0){
				  for(var i = 0;i<formt.length;i++){
					  htmls+= "</br><a  href='javascript:openParentTab(\""+formt[i].orgname+"重点预警\",\"<%=root%>/warning_search/toindexs?orgnum="+formt[i].deptno+"\")' style='text-decoration:none;' ><span class='content_t1'>"+formt[i].orgname+"产生"+formt[i].total+"条预警</span></a>";
				  	  if(formt.length >= disnum && i == disnum - 1){
						  flowDiv.innerHTML = htmls;
						  $G.parse();
						  return;
				  	  }
				  }
			  	  if(formt.length <=disnum){
					  flowDiv.innerHTML = htmls;
					  $G.parse();
			  	  }
			  }else{
	...
	function loadMyflow(){
		$.ajax({
			url: "<%=root%>/warning_search/getWarningInfo",
			data:null,
			cache: false,
			success: function (text) {
...
<%-- 		$.ajax({
	...
	function myFocus(){
        ...
        $G.showmodaldialog("添加/删除我的关注", "<%=root%>/myfocus/index", 700, 600, data, function(action){
	    	 grid.reload();
	    });
	}
	...
	function moreFocus(){
        ...
        $G.showmodaldialog("更多我的关注", "<%=root%>/myfocus/getMoreFocus", 600, 450, data, function(action){
	    	// grid.reload();
	    });
	<%-- 	$.ajax({
			url: "<%=root%>/warning_search/getMyFocus",
			data:null,
			cache: false,
			success: function (text) {
	  ...
	  //地图点击事件，跳转到运营全景视图-预警信息页面
		patternCharts.on('click', function (params) {
			//if(params.targetType && params.targetType == "axisName"){
				parent.createChialedTab({name:"运营全景视图",url:"<%=root%>/panorama/index?ywtypename="+params.name,title:"运营全景视图"})
			//} 
		});  
  	}
</script>
