	...
	<body>
        <div class="mini-fit" id= "bigDiv">
            <form id="form1" method="post" style="height: 100%;">
				<input class="mini-hidden" id="position_no"/>
            	<input class="mini-hidden" id="position_name"/>
						...
						<tr style="height: 100%">
							<td style="width: 40%; heigt:100%; border: #aaa solid 0px;">
								<fieldset style="border:solid 1px #aaa; height: 98%;">
									...
									<div id="tree1" class="mini-tree" style="width:100%;height:400px; padding:5px;"
									    ...
									    url="" dataField="datas" textField="name" idField="id" parentField="upid" >
               						</div>
								</fieldset>   
							</td>
							...
							<td style="width: 45%; height:100%; border: #aaa solid 0px;">
								<fieldset style="border:solid 0px #aaa;padding:0; height: 98%;">
									<legend>岗位&nbsp;<label style="color: red;" id="positionNameFill"></label>&nbsp;已分配人员：</legend>
									<div id="datagrid1" class="mini-datagrid" style="width:100%;height:96%;" onload = "gridonload"
										url="<%=root%>/org/getUsersByPositionId" dataField="datas" idfield="id" valuefield="id"
...
<script type="text/javascript">
	...
	function setData(data) {
   	    data = $G.clone(data);
   	    var position_no = data.id;
   	    var position_name = data.name;
   	    // 隐藏域保存岗位id和名称
   	    mini.get("position_no").setValue(position_no);
   	    mini.get("position_name").setValue(position_name);
   	    $("#positionNameFill").text(position_name);
   	 	$G.get("tree1").load("<%=root%>/org/getOrgUserList?position_no="+position_no);
   	    // 加载数据，查询所有具有position_no的人员
   	  	datagrid1.load({position_no:position_no});
   }
	...
	function removes(e) {
		var msgid;
  	  	var rows;
  	  	var delIds = [];
  	  	var position_no = mini.get("position_no").getValue();
  	  	var addGridRows = mini.get("addGridRows").getValue();
  	  	if(e == "1"){ // 选中的行
		...
		if(rows != null && rows.length > 0){
			...
			datagrid1.load({delIds:delIds,position_no:position_no});
		}else{
	...
	// 保存和删除选中的人员
	function SaveData(){
		// 获取数据，岗位id，选中的行，删除的数据
		var position_no = $G.get("position_no").getValue();
		var addGridRows = $G.get("addGridRows").getValue(); 
		var delIds = $G.get("delIds").getValue();
		...
		ajaxConf.setSuccessFunc(function (data) {
      	...
      	var urlStr = "<%=root%>/org/updatePositionUser";
		$G.postByAjax({position_no:position_no,saveIds:saveIds,delIds:delIds},urlStr,ajaxConf);
   }
