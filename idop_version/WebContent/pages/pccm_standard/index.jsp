...
<!-- 		                </td> -->
			...
			<div class="nui-fit">
		    	<div id="datagrid1" dataField="data" class="nui-datagrid" style="width:100%;height:100%;" url="<%=root%>/pccm_standard/getList" multiSelect="true">
			        <div property="columns">            
			            <div type="checkcolumn" name="checkCloumn"></div>
			            <div field="id" visible="false"></div>
			            <div field="standard_flag"  headerAlign="center" allowSort="true"  align="left">标准</div> 
			            <div field="second_org_num" allowSort="true" headerAlign="center" align="left">二级机构</div>
			            <div field="third_org_num" allowSort="true" headerAlign="center" align="left">三级机构</div>
			            <div field="create_time" allowSort="true" headerAlign="center" align="left" dateFormat="yyyy-MM-dd">创建日期</div>            
			        </div>
				</div>
			</div>
	</body>
...
// 	var form = $G.getForm("form1");
		...
		if(rows.length>0){
			$G.GcdsConfirm("确定删除选中记录？", "删除提示", function(action) {
				if (action == 'ok') {
                	...
                	$G.postByAjax({"ids":ids}, "<%=root%>/pccm_standard/del", ajaxConf);
              	}
            });
		}else{
			$G.alert("请选中一条记录");
	...
	//新增
	function add() {
        var url = "<%=root%>/pccm_standard/form";
        var bizParams = {pageType:"add"};
        $G.showmodaldialog("新增", url,1200, 500, bizParams, function(action){
	    	 grid.reload();
	    });
	}
	...
	function edit(){
		...
		if (row) {
	        ...
	        $G.showmodaldialog("编辑", url,1200, 500, bizParams, function(action){
		    	 grid.reload();
		    });
		}else{
			$G.alert("请先选中一条记录！");
