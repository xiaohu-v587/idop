...
<body>
        	...
        	<div style="padding-bottom: 20px; padding-top: 20px;">
          		<table style="table-layout: fixed;" align="center" >
          			<tr>
              			...
              			<td >
                			<input class="mini-textarea" name="mark_code"  height="25px" id="mark_code" width="210px" readonly="true"></input>
            			</td>
	...
	<script type="text/javascript">
	  ...
	  function saveJson() {
	    //保存
	     ...
	     var urlStr = "<%=root%>/datamart/updateMark";
	...
	function setData(data) {
		...
		$.ajax({
			url: "<%=root%>/datamart/getMarkByCode",
			data:{key:mark_code},
			cache: false,
			success: function (text) {
				var formd = mini.decode(text).record;
				var combobox = $G.get("sub_busi_code");
				var marktypecombobox = $G.get("mark_type_code");
				
				
				combobox.setUrl("<%=root%>/quotaDataExhibit/getSubbusicodeList?val="+formd.busi_module);
				marktypecombobox.setUrl("<%=root%>/quotaDataExhibit/getMarktypecodeList?val="+formd.busi_module);
				form.setData(formd); 
				var display_level = formd.display_level;
				var summary_level = formd.summary_level;
				...
				$G.getbyName("summary_level1").setValue(summary_level.split(",")[0]);
				$G.getbyName("summary_level2").setValue(summary_level.split(",")[1]);
				$G.getbyName("summary_level3").setValue(summary_level.split(",")[2]);
				$G.getbyName("summary_level4").setValue(summary_level.split(",")[3]);
